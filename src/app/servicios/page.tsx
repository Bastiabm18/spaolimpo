...
export default function Services ()  {
  ...
  return (
    <section id="servicios" className="w-full py-10 md:py-0 md:-mt-6 px-4 md:px-8">
      ...
      >
        ...
        {servicesData.map((service, index) => (
          ...
          onClick={() => router.push('/agenda')}
            key={index}
            className="
              flex flex-col items-center text-center p-8 md:p-2
          ...
          >
            {/* Ícono del servicio */}
            <service.icon className="h-14 w-14 md:h-7 md:w-7 mb-5 md:mb-2 text-[#D39154]" />

            {/* Título del servicio */}
            <h3 className="text-2xl md:text-lg font-bold text-neutral-200 mb-2 tracking-wide">
              {service.title}
            </h3>

            {/* Descripción del servicio */}
            <p className="font-alumniSans text-lg md:text:md text-neutral-400 mb-0 max-w-xs">
              {service.description}
            </p>
            ...
            <p
              className="
                mt-auto text-3xl md:text-xl font-bold text-[#D39154]
                bg-cover bg-clip-text text-transparent bg-[url('/madera2.jpeg')]
              "
